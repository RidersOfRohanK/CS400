...
public class SkipList<KeyType extends Comparable<KeyType>, ValueType> implements SortedMultiMap<KeyType, ValueType> {
  ...
  public KeyType getSmallestKey() {
    return this.head.getNext(0).key;
  }
  ...
  public Iterator<ValueType> getAll(KeyType key) {
    Iterator iterator = getRange(key, key);
    return iterator;
  }
  ...
  public Iterator<ValueType> getRange(KeyType start, KeyType end) {
    ...
    if (getSmallestKey().compareTo(start) == 1) {
      if (getSmallestKey().compareTo(end) == 1) {
        return null;
      } else {
        start = getSmallestKey();
      }
    }
    KeyType firstKey = start;

    if (getLargestKey().compareTo(end) == -1) {
      if (getLargestKey().compareTo(start) == -1) {
        return null;
      } else {
        end = getLargestKey();
      }
    ...
    SkipNode finalNode = getInternal(end);
    if (finalNode.key.equals(finalNode.getNext(0).key)) {
      while (finalNode.key.equals(finalNode.getNext(0).key)) {

        finalNode = finalNode.getNext(0);
        if (finalNode.getNext(0) == null) {
          break;
        }
    ...
    SkipNode lastNode = finalNode;
    Iterator iterator = new Iterator<>() {

      SkipNode current = getInternal(firstKey);
      ...
      public boolean hasNext() {
        if (current == null) {
          return false;
        } else if (current != null && current.equals(lastNode.getNext(0))) {
          return false;
        }
        return true;
      }
      ...
      public Object next() {
        if (!hasNext()) {
          ...
          ValueType returnVal = current.value;
          current = current.getNext(0);
